...
def provison_neptune_streams_handler(
    ...
    if lambda_runtime not in ["python3.6", "java8", "python", "java"]:
        raise Exception(
            "lambda_runtime must be python, java, python3.6 or java8: " + lambda_runtime
        )

    if lambda_logging_level not in ["DEBUG", "INFO", "WARN", "ERROR", "FATAL"]:
        raise Exception(
            "lambda_logging_level must be DEBUG, INFO, WARN, ERROR or FATAL: "
            + lambda_logging_level
        )

    if step_function_fallback_period_unit not in [
        "minutes",
        "minute",
        "hours",
        "hour",
        ...
        raise Exception(
            "step_function_fallback_period_unit must be minutes, minute, hours, hour, days or day: "
            + step_function_fallback_period_unit
        )

    if (
        step_function_fallback_period == 1
        and step_function_fallback_period_unit not in ["minute", "hour", "day"]
    ):
        raise Exception(
            "step_function_fallback_period_unit must be singular if step_function_fallback_period = 1: "
            + step_function_fallback_period_unit
        )
    ...
    if neptune_query_engine not in ["Sparql", "Gremlin"]:
        raise Exception(
            "query_engine must be Sparql or Gremlin: " + neptune_query_engine
        )
    ...
    stream_endpoint = "https://{}:{}/{}/stream".format(
        cluster_endpoint, port, query_engine.lower()
    )
    is_iam_auth_enabled = str(
        db_cluster.get("IAMDatabaseAuthenticationEnabled")
    ).lower()
    cluster_resource_id = db_cluster.get("DbClusterResourceId")

    security_group_ids = list(
        map(lambda s: s.get("VpcSecurityGroupId"), db_cluster.get("VpcSecurityGroups"))
    )
    ...
    subnet_group = describe_db_instances_response.get("DBInstances")[0].get(
        "DBSubnetGroup"
    )
    ...
    subnet_ids = list(
        map(lambda s: s.get("SubnetIdentifier"), subnet_group.get("Subnets"))
    )
    ...
    if len(route_tables) == 0:
        raise Exception(
            "You must explictly associate your subnets "
            + str(subnet_ids)
            + " to a Route Table"
        )
    ...
    vpc_endpoint_services = list(
        map(
            lambda s: s.get("ServiceName"),
            describe_vpc_endpoints_response.get("VpcEndpoints"),
        )
    )

    create_ddb_endpoint = str(
        not (any(e.endswith(".dynamodb") for e in vpc_endpoint_services))
    ).lower()
    create_monitoring_endpoint = str(
        not (any(e.endswith(".monitoring") for e in vpc_endpoint_services))
    ).lower()
    ...
    if not describe_vpc_attribute_response.get("EnableDnsSupport").get("Value"):
        raise Exception(
            "You must enable DNS resolution in your VPC in order to use the DynamoDB Gateway Endpoint"
        )
        ...
        tabulate(
            [
                ...
                [
                    "stream_endpoint",
                    stream_endpoint,
                    "Endpoint for source Neptune Stream. This is of the form http(s)://<cluster>:<port>/gremlin/stream or http(s)://<cluster>:<port>/sparql/stream.",
                ],
                [
                    "is_iam_auth_enabled",
                    is_iam_auth_enabled,
                    "Flag to determine if IAM Auth is enabled for source Neptune cluster.",
                ],
            ],
            headers="firstrow",
            tablefmt="fancy_grid",
        ...
        tabulate(
            [
                ...
                [
                    "handler_name",
                    handler_name,
                    "Name of the handler for processing stream records.",
                ],
                [
                    "additional_params",
                    additional_params_json,
                    "Additional params to be supplied to the handler via an AdditionalParams environment variable in the form of a JSON object.",
                ],
            ],
            headers="firstrow",
            tablefmt="fancy_grid",
        ...
        tabulate(
            [
                ...
                [
                    ...
                    lambda_memory_size_mb,
                    "Poller Lambda memory size (in MB).",
                ],
                [
                    "lambda_runtime",
                    lambda_runtime,
                    "Poller Lambda runtime (python3.6 or java8).",
                ],
                [
                    "lambda_logging_level",
                    lambda_logging_level,
                    "Poller Lambda logging level.",
                ],
                [
                    "managed_policies",
                    managed_policies,
                    "Comma-delimited list of ARNs of managed policies to be attached to Lambda execution role (for accessing other AWS resources from your handler).",
                ],
            ],
            headers="firstrow",
            tablefmt="fancy_grid",
        ...
        tabulate(
            [
                ...
                [
                    "batch_size",
                    batch_size,
                    "Number of records to be read from stream in each batch. Should be between 1 to 10000.",
                ],
                [
                    "max_polling_wait_time_seconds",
                    max_polling_wait_time_seconds,
                    "Maximum wait time in seconds between two successive polling from stream. Set value to 0 sec for continuous polling. Maximum value can be 3600 sec (1 hour).",
                ],
                [
                    "max_polling_interval_seconds",
                    max_polling_interval_seconds,
                    "Period for which we can continuously poll stream for records on one Lambda instance. Should be between 5 sec to 900 sec. This parameter is used to set Poller Lambda Timeout.",
                ],
                [
                    "step_function_fallback_period",
                    step_function_fallback_period,
                    "Period after which Step Function is invoked using CloudWatch Events to recover from failure.",
                ],
                [
                    "step_function_fallback_period_unit",
                    step_function_fallback_period_unit,
                    "Should be one of minutes, minute, hours, hour, days, day.",
                ],
                [
                    "create_cloudwatch_alarm",
                    str(create_cloudwatch_alarm).lower(),
                    "Flag used to determine whether to create CloudWatch alarm.",
                ],
                [
                    "notification_email",
                    notification_email,
                    "Email address for CloudWatch alarm notification.",
                ],
            ],
            headers="firstrow",
            tablefmt="fancy_grid",
        ...
        tabulate(
            [
                ...
                [
                    "route_table_ids",
                    route_table_ids,
                    "Comma-delimited list of route table ids associated with the Neptune subnets.",
                ],
                [
                    "create_ddb_endpoint",
                    create_ddb_endpoint,
                    "Flag used to determine whether to create DynamoDB VPC endpoint.",
                ],
                [
                    "create_monitoring_endpoint",
                    create_monitoring_endpoint,
                    "Flag used to determine whether to create Monitoring VPC endpoint.",
                ],
            ],
            headers="firstrow",
            tablefmt="fancy_grid",
    ...
    params = [
        ...
        {"ParameterKey": "AdditionalParams", "ParameterValue": additional_params_json},
        {"ParameterKey": "LambdaS3Bucket", "ParameterValue": handler_s3_bucket},
        {"ParameterKey": "LambdaS3Key", "ParameterValue": handler_s3_key},
        {
        ...
        {"ParameterKey": "LambdaLoggingLevel", "ParameterValue": lambda_logging_level},
        {"ParameterKey": "ManagedPolicies", "ParameterValue": managed_policies},
        {"ParameterKey": "StreamRecordsHandler", "ParameterValue": handler_name},
        {"ParameterKey": "StreamRecordsBatchSize", "ParameterValue": str(batch_size)},
        {
        ...
        {"ParameterKey": "NeptuneStreamEndpoint", "ParameterValue": stream_endpoint},
        {
        ...
        {"ParameterKey": "CreateDDBVPCEndPoint", "ParameterValue": create_ddb_endpoint},
        {
