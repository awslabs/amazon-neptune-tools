...
class NeptuneStreamToken:
    ...
    def after(event_id):
        return NeptuneStreamToken(
            last_event_id=event_id, iterator="AFTER_SEQUENCE_NUMBER"
        )
    ...
    def __init__(self, last_event_id=EventId(), iterator="TRIM_HORIZON"):
        ...
        assert iterator and iterator in [
            "AT_SEQUENCE_NUMBER",
            "AFTER_SEQUENCE_NUMBER",
            "TRIM_HORIZON",
            "LATEST",
        ], "iterator must be one of 'AT_SEQUENCE_NUMBER', 'AFTER_SEQUENCE_NUMBER', 'TRIM_HORIZON', 'LATEST'."
    ...
    def to_dict(self):
        return {
            "iteratorType": self.iterator,
            "commitNum": self.last_event_id.commit_num,
            "opNum": self.last_event_id.op_num,
        }
...
class NeptuneStreamRecord:
    ...
    def __init__(self, json, stream_format):
        ...
        self.data = (
            NeptuneStreamPropertyGraphRecord(json["data"])
            if stream_format in ["GREMLIN_JSON", "PG_JSON"]
            else NeptuneStreamRdfRecord(json["data"])
        )
...
class NeptuneStreamResponse:
    ...
    def __init__(self, json):
        ...
        self.last_event_id = EventId(
            json["lastEventId"]["commitNum"], json["lastEventId"]["opNum"]
        )
    ...
    def to_dict(self):
        return {"lastEventId": self.last_event_id.to_dict(), "records": self.records()}
...
class NeptuneStream:
    ...
    def __init__(self, endpoint_type="propertygraph", endpoints=None):
        ...
        assert endpoint_type and endpoint_type in [
            "sparql",
            "pg",
            "propertygraph",
            "gremlin",
        ], "endpoint_type must be one of 'sparql', 'pg', 'propertygraph', 'gremlin'."
    ...
    def all_records_for_commit(self, commit_num, batch_size=10):
        ...
        results = self.poll(
            NeptuneStreamToken.at(EventId(commit_num, 1)), limit=batch_size
        )
        allow_continue = True
