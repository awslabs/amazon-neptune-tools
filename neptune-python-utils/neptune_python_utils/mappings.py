...
class Separator:
    ...
    def split(self, s):
        ...
        return [
            v.replace(
                Separator.MULTI_VALUED_FIELD_ESCAPED_DELIMITER,
                Separator.MULTI_VALUED_FIELD_DELIMITER,
            )
            for v in values
        ]
...
class TokenMappings:
    ...
    def __init__(
        self, id_token="~id", label_token="~label", from_token="~from", to_token="~to"
    ):
    ...
    def is_token(self, s):
        return (
            self.is_id_token(s)
            or self.is_label_token(s)
            or self.is_from_token(s)
            or self.is_to_token(s)
        )
...
class Mapping:
    ...
    def __init__(
        ...
        if self.is_multi_valued and not self.separator:
            raise Exception(
                "No separator specified for multi-valued property: {}".format(self.key)
            )

    def is_id_token(self):
        return (
            self.token_mappings.is_id_token(self.key) if self.token_mappings else False
        )

    def is_label_token(self):
        return (
            self.token_mappings.is_label_token(self.key)
    ...
    def is_from_token(self):
        return (
            self.token_mappings.is_from_token(self.key)
            if self.token_mappings
            else False
        )
    ...
    def is_to_token(self):
        return (
            self.token_mappings.is_to_token(self.key) if self.token_mappings else False
        )
...
class Mappings:
    ...
    def mapping_for(self, key):
        ...
        if metadata:
            ...
            datatype = (
                metadata_match.group(1).lower() if metadata_match.group(1) else None
            )
            ...
            if datatype not in [
                ...
                if (
                    metadata_match.group(3)
                    and metadata_match.group(3).lower() == "single"
                ):
                    cardinality = "single"
                ...
                if metadata_match.group(4) and metadata_match.group(4) == "[]":
                    ...
                    if cardinality == "single":
                        raise Exception(
                            "Invalid mapping: single cardinality multi-valued property"
                        )
    ...
    def get_id(self, row):
        return (
            row[self.token_mappings.id_token]
            if self.token_mappings.id_token in row
            else None
        )

    def get_label(self, row):
        return (
            row[self.token_mappings.label_token]
            if self.token_mappings.label_token in row
            else None
        )

    def get_from(self, row):
        return (
            row[self.token_mappings.from_token]
            if self.token_mappings.from_token in row
            else None
        )

    def get_to(self, row):
        return (
            row[self.token_mappings.to_token]
            if self.token_mappings.to_token in row
            else None
        )
...
class TestMappings(unittest.TestCase):
    ...
    def test_throws_exception_if_multi_valued_single(self):
        ...
        try:
            ...
            self.assertEqual(
                str(e), "Invalid mapping: single cardinality multi-valued property"
            )
    ...
    def test_date_property(self):
        ...
        expected = datetime.datetime(
            2021, 6, 22, 12, 3, 52, tzinfo=datetime.timezone.utc
        )
