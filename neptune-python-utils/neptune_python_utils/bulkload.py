...
class BulkLoad:
    ...
    def __init__(
        ...
        **kwargs
        ...
        if region is None:
            assert (
                "AWS_REGION" in os.environ or "SERVICE_REGION" in os.environ
            ), "region is missing."
            self.region = os.environ.get(
                "SERVICE_REGION", os.environ.get("AWS_REGION", None)
            )
        else:
            self.region = region
    ...
    def __load(self, loader_endpoint, data):
        ...
        request_parameters = loader_endpoint.prepare_request(
            "POST", json_string, headers={"Content-Type": "application/json"}
        )
    ...
    {} -d \'{}\'""".format(loader_endpoint, json.dumps(json_payload, indent=4)))
        ...
        return BulkLoadStatus(
            self.endpoints.load_status_endpoint(load_id), **self.kwargs
        )
...
class BulkLoadStatus:
    ...
    def status(self, details=False, errors=False, page=1, errors_per_page=10):
        ...
        request_parameters = self.load_status_endpoint.prepare_request(
            querystring=params
        )
    ...
    def wait(self, interval=2):
        while True:
            ...
            if status == "LOAD_IN_PROGRESS":
                print(
                    "loading... {} records inserted".format(
                        json_response["payload"]["overallStatus"]["totalRecords"]
                    )
                )
                time.sleep(interval)
            else:
