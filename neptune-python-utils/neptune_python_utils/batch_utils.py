...
def add_property_to_vertex(t, mapping, value):
    ...
    if cardinality == Cardinality.set_ and mapping.is_multi_valued:
        col = (
            ...
            else mapping.separator.split(value)
        )
        for s in col:
...
def add_property_to_edge(t, mapping, value):
    ...
    if type(value) in [list, set, tuple]:
        raise Exception(
            "Unsupported list/set/tuple type for edge property: {}".format(mapping.name)
        )
...
def upsert_vertex(t, row, **kwargs):
    ...
    for key, value in row.items():
        ...
        if mapping.is_id_token():
            ...
            if not on_upsert:
                create_traversal = add_property_to_vertex(
                    create_traversal, mapping, value
                )
...
def add_edge(t, row, **kwargs):
    ...
    t = (
        t.addE(label)
        .from_(V(mappings.get_from(row)))
        .to(V(mappings.get_to(row)))
        .property(id, mappings.get_id(row))
    )
...
def upsert_edge(t, row, **kwargs):
    ...
    if not on_upsert:
        for key, value in row.items():
            mapping = mappings.mapping_for(key)
            if not mapping.is_token():
                create_traversal = add_property_to_edge(
                    create_traversal, mapping, value
                )

    t = (
        t.V(mappings.get_from(row))
        .outE(label)
        .hasId(mappings.get_id(row))
        .fold()
...
def reset_connection_if_connection_issue(params):
    ...
    print(
        "is_reconnectable: {}, num_tries: {}".format(is_reconnectable, params["tries"])
    )
    ...
    if is_reconnectable:
        try:
            ...
            print(
                "[{}] Error closing connection: {}".format(pid, traceback.format_exc())
            )
        finally:
...
def publish_metrics(invocation_details):
    ...
    if retries > 0:
        print(
            "Retries: {}, Elapsed: {}s".format(retries, invocation_details["elapsed"])
        )
    ...
    if job_name and retries > 0:
        ...
        cloudwatch.put_metric_data(
            MetricData=[
                {
                    "MetricName": "Retries",
                    "Dimensions": [{"Name": "client", "Value": job_name}],
                    "Unit": "Count",
                    "Value": float(retries),
                },
            ],
...
class BatchUtils:
    ...
    def __init__(
        self, endpoints, job_name=None, to_dict=lambda x: x, pool_size=1, **kwargs
    ):
    ...
    def add_vertices(self, batch_size=50, rows=None, **kwargs):
        def batch_op(rows):
            self.execute_batch(
                rows, operations=[add_vertex], batch_size=batch_size, **kwargs
            )
    ...
    def upsert_vertices(self, batch_size=50, rows=None, **kwargs):
        def batch_op(rows):
            ...
            self.execute_batch(
                rows, operations=operations, batch_size=batch_size, **kwargs
            )
    ...
    def add_edges(self, batch_size=50, rows=None, **kwargs):
        def batch_op(rows):
            self.execute_batch(
                rows, operations=[add_edge], batch_size=batch_size, **kwargs
            )
